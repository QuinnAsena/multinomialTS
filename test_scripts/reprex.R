...
## Original version that returns a single value
mnTS_ml_cpp(par = par.start, par_fixed = par.fixed, Y = Y, X = X, Tsample = 1:ncol(X))

## Altering Y so that it the function should return 1e+10
## Original version that returns a double
mnTS_ml_cpp(par = par.start, par_fixed = par.fixed, Y = matrix(0, nrow = 3, ncol = 140), X = X, Tsample = 1:ncol(X))
## Version with Rcpp::List return
